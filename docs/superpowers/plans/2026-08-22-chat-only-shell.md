...
- **Five debts inherited from stage C5 are untouched, and all five are now written down in spec §11** rather than only in the tail of the C5 plan: `search_notes` still returns no `source_type` so a saved answer is cited like the user's own thinking (§11.1); the declined-offer exclusion has no consumer to be proven against (§11.2, S4's to close); `OFFER_DEDUP_THRESHOLD` is still the unmeasured `0.88` in `offer.ts` (§11.3); verification quality is unmeasured (§11.4); and `FORMAT_RULE` obedience is a human check, both halves of it (§11.5). Two further C5 debts are **closed** rather than carried: mobile markdown is re-attempted here in Tasks 13–14, and multi-session scrollback plus mobile chat history are what Tasks 3–5 and 10–12 build.
